...
{
    #pragma HLS INTERFACE m_axi port=input bundle=mem0 depth=4194304 offset=slave
#pragma HLS INTERFACE m_axi port=input_local bundle=mem0 depth=4194304 offset=slave

#pragma HLS INTERFACE m_axi port=block0_weights_conv1 bundle=mem1 depth=524288 offset=slave
#pragma HLS INTERFACE m_axi port=block0_weights_ds_conv1 bundle=mem1 depth=524288 offset=slave

#pragma HLS INTERFACE m_axi port=block0_output_conv1 bundle=mem2 depth=524288 offset=slave
#pragma HLS INTERFACE m_axi port=block0_output_ds_conv1 bundle=mem2 depth=524288 offset=slave

#pragma HLS INTERFACE m_axi port=block0_gamma1 bundle=mem3 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block0_beta1 bundle=mem3 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block0_mean1 bundle=mem3 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block0_var1 bundle=mem3 depth=512 offset=slave

#pragma HLS INTERFACE m_axi port=block0_output_bn1 bundle=mem4 depth=524288 offset=slave
#pragma HLS INTERFACE m_axi port=block0_output_bn2 bundle=mem4 depth=524288 offset=slave

#pragma HLS INTERFACE m_axi port=block0_weights_conv2 bundle=mem5 depth=524288 offset=slave

#pragma HLS INTERFACE m_axi port=block0_gamma2 bundle=mem6 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block0_beta2 bundle=mem6 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block0_mean2 bundle=mem6 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block0_var2 bundle=mem6 depth=512 offset=slave

#pragma HLS INTERFACE m_axi port=output_local bundle=mem7 depth=524288 offset=slave
#pragma HLS INTERFACE m_axi port=output_block0 bundle=mem7 depth=524288 offset=slave

#pragma HLS INTERFACE m_axi port=block1_weights_conv1 bundle=mem8 depth=524288 offset=slave
#pragma HLS INTERFACE m_axi port=block1_weights_conv2 bundle=mem8 depth=524288 offset=slave

#pragma HLS INTERFACE m_axi port=block1_output_conv1 bundle=mem9 depth=524288 offset=slave
#pragma HLS INTERFACE m_axi port=block1_output_conv2 bundle=mem9 depth=524288 offset=slave

#pragma HLS INTERFACE m_axi port=block1_gamma1 bundle=mem10 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block1_beta1 bundle=mem10 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block1_mean1 bundle=mem10 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block1_var1 bundle=mem10 depth=512 offset=slave

#pragma HLS INTERFACE m_axi port=block1_gamma2 bundle=mem11 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block1_beta2 bundle=mem11 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block1_mean2 bundle=mem11 depth=512 offset=slave
#pragma HLS INTERFACE m_axi port=block1_var2 bundle=mem11 depth=512 offset=slave

#pragma HLS INTERFACE m_axi port=block1_output_bn1 bundle=mem12 depth=524288 offset=slave
#pragma HLS INTERFACE m_axi port=block1_output_bn2 bundle=mem12 depth=524288 offset=slave

#pragma HLS INTERFACE m_axi port=output_avgpooling bundle=mem13 depth=131072 offset=slave
#pragma HLS INTERFACE m_axi port=fc_output bundle=mem13 depth=131072 offset=slave
#pragma HLS INTERFACE m_axi port=tanh_output bundle=mem13 depth=131072 offset=slave

#pragma HLS INTERFACE m_axi port=fc_weights bundle=mem14 depth=262144 offset=slave
#pragma HLS INTERFACE m_axi port=fc_bias bundle=mem14 depth=512 offset=slave
    ...
    bool relu) // ReLU activation flag
{
    ...
    for (int b = 0; b < BATCH; ++b)
    {
        for (int oc = 0; oc < OUT_CHANNELS; ++oc)
        {
            ...
            data_t gamma_local = gamma[oc] * inv_std;
            data_t mean_local = mean[oc];
            //can load weights per out channel here
           
            data_t local_weights[IN_CHANNELS][KERNEL_HEIGHT][KERNEL_WIDTH];
            #pragma HLS ARRAY_PARTITION variable=local_weights complete
            load_weights_per_out_channel(weights[oc], local_weights); // Load weights for conv1

            for (int i = 0; i < HEIGHT; i += 2)
            {//Weights are reused per i,j. Should load ic, kh, kw 
                //can load inputs here
                    ...
                    { // loop over input channels
                        for (int kh = 0; kh < KERNEL_HEIGHT; ++kh)
                        {
                            ...
                            {   
                                ...
                                int in_h = i + kh - PADDING;//new i
                                int in_w = j + kw - PADDING;//new j
    ...
    data_t output[BATCH][OUT_CHANNELS])
{
    ...
    // First do over batch
	//can unroll here too depending on resource usage
    for (int b = 0; b < BATCH; b++)
    {
		//#pragma HLS PIPELINE II=1
        ...
        for (int j = 0; j < OUT_CHANNELS; j++)
        {
            ...
            {   //can add unroll here
                ...
                // k
                #pragma HLS UNROLL factor=UNROLL_FACTOR
                sum+= input_local[b][k] * weights[j][k]; // If wrong maybe try swapping k with j
            }
            sum += bias[j]; // Add bias to the output
                                     // apply tanh
			//added a variable for sum^2 (used twice)
